var BOT = BOT || function() {
	...
	function total_pieces(board) {
		return applyFun(board, 0, function(acc, elem) {
			if(!elem || !elem.type) return acc;			
			var res = acc + value_elem(elem) * (side === elem.side ?	1 : -1);
			if(res === undefined){
	...
	*/
	...
	function good_steps(steps, board, gamestate) {
		...
		var values = GENERIC.map(steps, function(step) {
			...
			return total_pieces(after.board);
		});
	...
	function deep_steps(board, gamestate) {
		...
		for(var i = 0; i < tries; ++i) {
			...
			if(ARIMAA.is_gameover(after.board, after.gamestate)) {
				...
				var value = 
					positional_value(after.board) + total_pieces(after.board);
				if(!best || value > best) {
	...
	function get_move(board, gamestate) {
		var goal = try_to_goal(board, gamestate);
		if(!!goal) {
			var steps = goal;
		} else {
			var steps = deep_steps(board, gamestate);
		}
